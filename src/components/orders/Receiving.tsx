    ...
    <div className="max-w-7xl mx-auto space-y-6 lg:space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end border-b border-slate-800 pb-6 lg:pb-8 gap-4">
        <div>
          ...
          <h1 className="text-2xl lg:text-3xl font-black text-white italic">入荷管理 <span className="text-slate-600 text-lg lg:text-xl font-light">/ Receiving</span></h1>
        </div>
        <button
          onClick={handleAddNew}
          className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-xl lg:rounded-2xl font-black text-[10px] lg:text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-900/20"
        >
                ...
                <tr key={r.id} className="group hover:bg-slate-800/10 transition-colors">
                  ...
                  <td className="py-4 px-6 text-center">
                    {editingId === r.id ? (
                      ...
                      <span className={`text-[10px] font-black px-2 py-1 rounded uppercase tracking-tighter ${r.status === '入荷済' ? 'bg-emerald-500/10 text-emerald-500' :
